...
window.RoboticsExerciseComponents = (function() {
   ...
   */
  const renderComponentNew = function(component, container_id, properties, children_elements) {
  ...
  const renderImportNew = async function(element, parent_is_root) {
    ...
    if(path[0]==='exercise') {
      ...
      rendered_component = await import(`exercises/${component_path}`).then((component) => {
        return renderComponentNew(component.default, dom_id, properties, child_elements);
      })
    } else {
      rendered_component = await import(`/${component}.js`).then((component) => {
        return renderComponentNew(component.default, dom_id, properties, child_elements);
      });
    }
