...
class Config:
    def __init__(self):
        with open('/RoboticsAcademy/exercises/static/exercises/laser_mapping/config_sim.json') as file:
        ...
        self.orientation = math.radians(
            self.config['CONFIGURATION']['ORIENTATION'])
        ...
        if (os.path.basename(file.name) == 'config_real.json'):
            ...
            self.msg = 0
            self.pub_motors = rospy.Publisher(
                self.topic_enable_motors, Int32, queue_size=1)
