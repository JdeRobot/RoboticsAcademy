...
class BrowserInputImagePublisher(Node):
    ...
    def __init__(self, topic_name="/input/image_raw"):
        super().__init__("basic_computer_vision_input_publisher")
...
class WebGUI(MeasuringThreadingGUI):
    def __init__(self, host="ws://127.0.0.1:2303"):
        ...
        # Last encoded frame, re-sent when idle so the GUI does not flicker
        self.last_image_payload = {"image": "", "shape": ""}
    ...
    def payloadImage(self):
        ...
        # Nothing new since last send: re-send the last frame so the GUI
        # keeps showing it instead of flickering to a blank image.
        if not display_image_updated:
            return self.last_image_payload

        shape = display_image.shape
        frame = cv2.imencode(".JPEG", display_image)[1]
        encoded_image = base64.b64encode(frame)

        payload = {"image": encoded_image.decode("utf-8"), "shape": shape}

        with self.display_image_lock:
            self.display_image_updated = False

        self.last_image_payload = payload
        return payload
