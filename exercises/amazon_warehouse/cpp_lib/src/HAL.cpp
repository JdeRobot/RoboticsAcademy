...
// Hidden platform control variables to avoid polluting HAL.hpp with ROS 2 messages
static std::shared_ptr<rclcpp::Node> platform_node_ = nullptr;
static std::shared_ptr<rclcpp::Publisher<std_msgs::msg::Float64>> platform_pub_ = nullptr;

void HAL::init()
{
    if (!motors_node_) {
        motors_node_ = std::make_shared<MotorsNode>("/amazon_robot/cmd_vel", 4.0, 0.3, "hal_motors");
        odometry_node_ = std::make_shared<OdometryNode>("/amazon_robot/odom", "hal_odom");
        laser_node_ = std::make_shared<LaserNode>("/amazon_robot/scan", "hal_laser");
        ...
        platform_pub_ = platform_node_->create_publisher<std_msgs::msg::Float64>("/platform/cmd_vel", 10);
