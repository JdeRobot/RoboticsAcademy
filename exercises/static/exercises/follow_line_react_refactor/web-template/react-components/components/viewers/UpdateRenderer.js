function UpdateRenderer()
{
  ...
  this.drawAerialView = () => {
    if(circuitImageLoaded) {
      this.ctx.drawImage(circuitImage, circuitMarginX, circuitMarginY, this.circuitWidth, this.circuitHeight);

      if(this.updateRef.current.map)
      {
        const position = this.updateRef.current.map.slice(1,).slice(0,-1).split(',');
