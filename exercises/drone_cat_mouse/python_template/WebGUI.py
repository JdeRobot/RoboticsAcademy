...
class ROS2BridgeNode(Node):
    def __init__(self, gui_instance):
        super().__init__("gui_bridge_node_drone_cat_mouse")
...
class WebGUI(MeasuringThreadingGUI):
    ...
    def update_gui(self):
        if np.any(self.left_image):
            _, encoded_left = cv2.imencode(".JPEG", self.left_image)
            b64_left = base64.b64encode(encoded_left).decode("utf-8")
        ...
        if np.any(self.right_image):
            _, encoded_right = cv2.imencode(".JPEG", self.right_image)
            b64_right = base64.b64encode(encoded_right).decode("utf-8")
