...
class PerceptionManager(Node):
    ...
    def __init__(self):
        ...
        self.color_filter_pub = self.create_publisher(
            ColorFilter, "/start_color_filter", 10
        )
        self.shape_filter_pub = self.create_publisher(
            ShapeFilter, "/start_shape_filter", 10
        )
                ...
                for name, spec in objects.items():
                    ...
                    if shape == "box":
                        ...
                        self.object_list[name] = Object(
                            p.pose, object_pose, sz, sy, sx, shape, color
                        )
                    elif shape == "cylinder":
...
def attach(item):
    ...
    link_map = {
        ...
        "green_sphere": "green_sphere",
        "purple_sphere": "purple_sphere",
        "blue_cylinder": "blue_cylinder",
        "red_cylinder": "red_cylinder",
        "green_cylinder": "green_cylinder",
        "purple_cylinder": "purple_cylinder",
    }
    ...
    for name, spec in objects.items():
        ...
        if shape == "box":
            ...
            object_list[name] = Object(
                p.pose, None, height, radius * 2.0, radius * 2.0, shape, color
            )
            ...
            object_list[name] = Object(
                p.pose, None, radius * 2.0, radius * 2.0, radius * 2.0, shape, color
            )
