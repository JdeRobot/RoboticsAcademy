...
public:
  WebGUINode()
      : BaseWebGUI("webgui_node", "127.0.0.1", "2303", 30.0),
        ...
        lift_state_(false), array_str_("[]"),
        map_util_([this]() { return odom_node_->getPose3d(); }) {
    odom_node_ =
        std::make_shared<OdometryNode>("/logistic_robot/odom", "webgui_odom");
  ...
  void show_path(const std::vector<std::vector<double>> &array) {
    // In C++, we use nlohmann::json directly instead of regular expressions
    // like in Python
    ...
    std::lock_guard<std::mutex> lk(array_mtx_);
    array_str_ =
        j_array.dump(); // Generates a string in the format "[[x,y],[x,y]]"
  }
      ...
      for (int x = 0; x < image.cols; ++x) {
        ...
        if (val < 128) {
          ...
          case 132:
            ...
            break; // blue
          case 133:
            color = cv::Vec3b(130, 0, 75);
            break; // indigo
          case 134:
            color = cv::Vec3b(211, 0, 148);
            break; // violet
          default:
