...
class Launch(Tests):
    ...
    def main(self):
        try:
            args = ['/opt/ros/noetic/bin/roslaunch', '/RoboticsAcademy/exercises/drone_cat_mouse/web-template/launch/gazebo.launch', '--wait']
            ...
            args = ['/opt/ros/noetic/bin/roslaunch', '/RoboticsAcademy/exercises/drone_cat_mouse/web-template/launch/px4_cat.launch']
            if ACCELERATION_ENABLED: args.insert(0, 'vglrun')
            subprocess.Popen(args, stdout=subprocess.PIPE)
            ...
            args = ['/opt/ros/noetic/bin/roslaunch', '/RoboticsAcademy/exercises/drone_cat_mouse/web-template/launch/mavros_cat.launch']
            if ACCELERATION_ENABLED: args.insert(0, 'vglrun')
            subprocess.Popen(args, stdout=subprocess.PIPE)

            rospy.logwarn('[ MAVROS ] [ cat ] Waiting for mavros')
            self.test_mavros('/cat')

            args = ['/opt/ros/noetic/bin/roslaunch', '/RoboticsAcademy/exercises/drone_cat_mouse/web-template/launch/px4_mouse.launch']
            if ACCELERATION_ENABLED: args.insert(0, 'vglrun')
            subprocess.Popen(args, stdout=subprocess.PIPE)
            ...
            args = ['/opt/ros/noetic/bin/roslaunch', '/RoboticsAcademy/exercises/drone_cat_mouse/web-template/launch/mavros_mouse.launch']
            if ACCELERATION_ENABLED: args.insert(0, 'vglrun')
            subprocess.Popen(args, stdout=subprocess.PIPE)

            rospy.logwarn('[ MAVROS ] [ mouse ] Waiting for mavros')
