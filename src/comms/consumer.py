...
class ManagerConsumer:
    ...
    async def handler(self, websocket: WebSocketServerProtocol):
        ...
        if self.client is not None and websocket != self.client:
            logger.debug("Client already connected, rejecting connection")
        ...
        if self.client and self.client.closed:
            logger.debug("Client disconnected, machine state reset")
            self.manager.reset()
            self.client = None
            return
    ...
    def start(self):
        ...
        logger.debug(f"Websocket server listening in {self.host}:{self.port}")
