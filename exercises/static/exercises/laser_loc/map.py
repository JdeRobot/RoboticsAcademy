...
class Map:
    ...
    def setRobotValues(self):
        ...
        xg, yg = self.relative2global(
            x_desired + (x - self.initial_x),
            y_desired + (y - self.initial_y),
            xr,
            yr,
            yaw_desired + (yaw - self.initial_yaw),
        )
        ...
        xc, yc = self.global2canvas(
            x_desired + (x - self.initial_x), y_desired + (y - self.initial_y)
        )
    ...
    def setSonarValues(self):
        ...
        self.sonares[0] = (
            self.config.sonar_0["POS_X"],
            self.config.sonar_0["POS_Y"],
            self.config.sonar_0["ORIENTATION"],
        )
        self.sonares.append((0, 0, 0))
        self.sonares[1] = (
            self.config.sonar_1["POS_X"],
            self.config.sonar_1["POS_Y"],
            self.config.sonar_1["ORIENTATION"],
        )
        self.sonares.append((0, 0, 0))
        self.sonares[2] = (
            self.config.sonar_2["POS_X"],
            self.config.sonar_2["POS_Y"],
            self.config.sonar_2["ORIENTATION"],
        )
        self.sonares.append((0, 0, 0))
        self.sonares[3] = (
            self.config.sonar_3["POS_X"],
            self.config.sonar_3["POS_Y"],
            self.config.sonar_3["ORIENTATION"],
        )
        self.sonares.append((0, 0, 0))
        self.sonares[4] = (
            self.config.sonar_4["POS_X"],
            self.config.sonar_4["POS_Y"],
            self.config.sonar_4["ORIENTATION"],
        )
        self.sonares.append((0, 0, 0))
        self.sonares[5] = (
            self.config.sonar_5["POS_X"],
            self.config.sonar_5["POS_Y"],
            self.config.sonar_5["ORIENTATION"],
        )
        self.sonares.append((0, 0, 0))
        self.sonares[6] = (
            self.config.sonar_6["POS_X"],
            self.config.sonar_6["POS_Y"],
            self.config.sonar_6["ORIENTATION"],
        )
        self.sonares.append((0, 0, 0))
        self.sonares[7] = (
            self.config.sonar_7["POS_X"],
            self.config.sonar_7["POS_Y"],
            self.config.sonar_7["ORIENTATION"],
        )
        # SONAR 0
        self.dist.append(0)
        ...
        for i in range(8):
            ...
            if self.sonares[i][2] < 0:
                ...
                xr_1, yr_1 = self.local2relative(
                    self.sonares[i][0], self.sonares[i][1], hipotenusa, orientation
                )
                orientation = self.sonares[i][2] + cone / 2
                xr_2, yr_2 = self.local2relative(
                    self.sonares[i][0], self.sonares[i][1], hipotenusa, orientation
                )
            else:
                orientation = self.sonares[i][2] + cone / 2
            ...
            xg_1, yg_1 = self.relative2global(
                x_desired + (x - self.initial_x),
                y_desired + (y - self.initial_y),
                xr_1,
                yr_1,
            ...
            xg_2, yg_2 = self.relative2global(
                x_desired + (x - self.initial_x),
                y_desired + (y - self.initial_y),
                xr_2,
                yr_2,
    ...
    def setLaserValues(self):
        ...
        xg, yg = self.relative2global(
            x_desired + (x - self.initial_x),
            y_desired + (y - self.initial_y),
            self.config.laser["POS_X"],
            self.config.laser["POS_Y"],
            yaw_desired + (yaw - self.initial_yaw),
        )
        xc, yc = self.global2canvas(xg, yg)
        laser_sensor = [xc, yc]
        ...
        for i in range(0, cont_rays - 1):
            ...
            if dist == float("inf"):
                dist = laser.maxRange
            xr, yr = self.local2relative(
                self.config.laser["POS_X"], self.config.laser["POS_Y"], dist, angle
            )
            xg, yg = self.relative2global(
                x_desired + (x - self.initial_x),
                y_desired + (y - self.initial_y),
