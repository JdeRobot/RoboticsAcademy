...
class ListenerParameters:
    def __init__(self, configFile, cam):
        ...
        if os.getcwd() == "/":
            f = open("/workspace/code/" + configFile, "r")
        else:
            f = open(configFile, "r")
    ...
    def project(self, point3d):
        point3d[3] = -1.0
        a1 = (
            self.RT[0, 0] * point3d[0]
            + self.RT[0, 1] * point3d[1]
            + self.RT[0, 2] * point3d[2]
            + self.RT[0, 3] * point3d[3]
        )
        a2 = (
            self.RT[1, 0] * point3d[0]
            + self.RT[1, 1] * point3d[1]
            + self.RT[1, 2] * point3d[2]
            + self.RT[1, 3] * point3d[3]
        )
        a3 = (
            self.RT[2, 0] * point3d[0]
            + self.RT[2, 1] * point3d[1]
            + self.RT[2, 2] * point3d[2]
            + self.RT[2, 3] * point3d[3]
        )
        a4 = (
            self.RT[3, 0] * point3d[0]
            + self.RT[3, 1] * point3d[1]
            + self.RT[3, 2] * point3d[2]
            + self.RT[3, 3] * point3d[3]
        )
