...
from hal_interfaces.general.noise_odometry import (
    Pose3d, quat2Yaw, quat2Pitch, quat2Roll, 
    gaussian_noise, add_noise, 
    euler2quat, NoisyOdometryNode
)
...
class TestGaussianNoise(unittest.TestCase):
    ...
    def test_average_noise_is_close_to_mean(self):
        ...
        noisy_values = [gaussian_noise(value, mu=mean, noise_level=0.1) for _ in range(samples)]
...
class TestNoisyOdometryNode(unittest.TestCase):
    ...
    def test_initialization(self, mock_init):
        ...
        with patch('hal_interfaces.general.noise_odometry.Node.create_subscription', 
    ...
    def test_noise_accumulation(self, mock_init):
        ...
        with patch('hal_interfaces.general.noise_odometry.Node.create_subscription', 
                  return_value=MagicMock()):
            with patch('hal_interfaces.general.noise_odometry.add_noise') as mock_add_noise:
                ...
                def side_effect(prev_odom, odom, base_odom, noise_level):
                    ...
                    position_index = int(odom.pose.pose.position.x)  # Use x as index (1.0, 2.0, etc)
                    result = MockOdometry(
                        x=odom.pose.pose.position.x + 0.05 * position_index,  # More noise for later positions
