...
std::shared_ptr<NeuralNetwork> HAL::neural_network_ = nullptr;
...
HAL::LaserData HAL::get_laser_data()
{
    ...
    while (raw_laser.timeStamp == 0.0 && rclcpp::ok()) {
...
std::vector<HAL::BoundingBox> HAL::get_bounding_boxes(const cv::Mat& image)
{
    std::vector<HAL::BoundingBox> bounding_boxes;

    if (!neural_network_ || image.empty()) {
        return bounding_boxes;
    }

    auto raw_bounding_boxes = neural_network_->getBoundingBoxes(image);

    for (const auto& raw_box : raw_bounding_boxes) {
        HAL::BoundingBox bounding_box{
            raw_box.id,
            raw_box.class_id,
            raw_box.score,
            raw_box.xmin,
