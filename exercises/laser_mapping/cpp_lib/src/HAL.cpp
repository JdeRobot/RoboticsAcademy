...
void HAL::init() {
  if (!motors_node_) {
    odometry_node_ =
        std::make_shared<OdometryNode>("/do150/odom", "hal_odometry");
    noisy_odometry_node_ = std::make_shared<OdometryNode>(
        "/do150/odom_noisy", "hal_noisy_odometry_node");
    motors_node_ =
        std::make_shared<MotorsNode>("/do150/cmd_vel", 4.0, 0.3, "hal_motors");
    laser_node_ = std::make_shared<LaserNode>("/do150/laser/scan", "hal_laser");
