...
class PlotWidget(QtWidgets.QWidget):
    ...
    def update_figure(self, pred, actual):
        if len(pred) > 0 and len(actual) > 0:
            self.pred_plot.setData(
                pred[:, 0], pred[:, 1], pen=pg.mkPen("r", width=2), antialias=True
            )
            self.actual_plot.setData(
                actual[:, 0], actual[:, 1], pen=pg.mkPen("b", width=2), antialias=True
            )
