...
function startSim(step, map="default", websocket_address="",server="", username=""){
    var level = 0;
    ...
    if (step == 0) {
        ...
        ws_manager.send(JSON.stringify({
            "command": "open", "exercise": exercise, "width": size.width.toString(), "height": size.height.toString(), "map": map}));
        level++;
        connectionUpdate({connection: 'exercise', command: 'launch_level', level: `${level}`}, '*');
    ...
    ws_manager.onclose = function (event) {
        ...
        if (!first_attempt) {
            ...
            startSim(step, map, websocket_address,server, username);
        } else {
            first_attempt = false;
        }
    }
