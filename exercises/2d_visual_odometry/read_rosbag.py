...
class Read_Rosbag:
    def __init__(self, pose_obj, file_dir, myGUI):
        ...
        self.orientation_noise = np.random.normal(
            0, (1 / 3), [orientation_count, 2]
        ).tolist()  # orientation noise mu =0 and 5*sigma = 1
    ...
    def align(self, model, data):
        ...
        if model.shape[1] > data.shape[1]:
            ...
            remove = np.linspace(
                start=1, stop=model.shape[1], num=diff, endpoint=False, dtype=int
            )
            ...
            remove = np.linspace(
                start=1, stop=data.shape[1], num=diff, endpoint=False, dtype=int
            )
        ...
        if np.linalg.det(U) * np.linalg.det(Vh) < 0:
            S[2, 2] = -1
        rot = U * S * Vh
        trans = (data.mean(1).reshape((data.mean(1).shape[0]), 1)) - rot * (
            model.mean(1).reshape((model.mean(1).shape[0]), 1)
        )
    ...
    def depthToRGB8(self, float_img_buff, encoding):
        ...
        if encoding[-3:-2] == "U":
            ...
            float_img = np.zeros(
                (float_img_buff.shape[0], float_img_buff.shape[1], 1), dtype="float32"
            )
    ...
    def next_message(self, sensors):
        ...
        while True:
            ...
            t = t.to_sec()
            if topic == "/imu" and (
                "accelerometer" in sensors or sensors == () or "stream" in sensors
            ):
            ...
            elif topic == "/camera/rgb/image_color" and (
                "color_img" in sensors or sensors == () or "stream" in sensors
            ):
                self.color_img = self.bridge.imgmsg_to_cv2(msg, desired_encoding="rgb8")
                self.color_img_t = t
                self.topic_name = "color_img"

            elif topic == "/camera/depth/image" and (
                "depth_img" in sensors or sensors == () or "stream" in sensors
            ):
                ...
                "scan" in sensors or sensors == () or "stream" in sensors
            ):
