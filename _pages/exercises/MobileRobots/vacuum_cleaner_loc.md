...
- `HAL.getBumperData().state` - To establish if the robot has crashed or not. Returns a 1 if the robot collides and a 0 if it has not crashed. **DISABLED**: use laser data in the center (90) to get the same result.
- `HAL.getBumperData().bumper` - If the robot has crashed, it turns to 1 when the crash occurs at the center of the robot, 0 when it occurs at its right and 2 if the collision is at its left. **DISABLED**: use laser data in the center (90) to get the same result.
...
2. [https://en.wikipedia.org/wiki/Transformation\_(function)](<https://en.wikipedia.org/wiki/Transformation_(function)>)
