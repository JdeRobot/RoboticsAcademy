...
class Pose:
    ...
    def set_actual_pose(self, pos, time):
        ...
        self.actual_pose = [
            pos[0] - self.init_actual_pose[0],
            pos[1] - self.init_actual_pose[1],
        ]
        self.actual_path_time.append(time)
        self.actual_path_without_interp.append(self.actual_pose)
        if len(self.pred_path_time) != 0:
            self.actual_path_x = np.interp(
                self.pred_path_time,
                self.actual_path_time,
                [k[0] for k in self.actual_path_without_interp],
            )
            self.actual_path_y = np.interp(
                self.pred_path_time,
                self.actual_path_time,
                [k[1] for k in self.actual_path_without_interp],
            )
