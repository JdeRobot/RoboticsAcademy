...
class Template:
    ...
    def process_code(self, source_code):
        ...
        try:
            ...
            img_shape = img.shape
            payload = {"image": "" , "shape": ""}
            encode = cv2.imencode(".PNG",img)[1]
            encoded_image = base64.b64encode(encode)
            payload["image"] = encoded_image.decode('utf-8')
            payload['shape'] = img_shape
              
            self.pay_load["image"] = json.dumps(payload)
            
            strArray = np.array_str(array)
   
            if re.search(r"[{1,3}[ ]+",strArray):
               strArray = re.sub(r"\[[ ]+","[",strArray)
            strArray = re.sub(r"[ ]+",", ",strArray)
            if re.search(r",[ ]+]",strArray):
               strArray = re.sub(r",[ ]+]","]",strArray)
            print(strArray)
            
            
            self.pay_load["array"] = strArray
            
            nump = np.array(str(array))
            message = "#arry" + json.dumps(self.pay_load)
            #num1 = np.array([[[90.87462062085475,45.9746815056445,2494.80004882813],[6.87473799526779,45.9772917433492,2517.48754882813]],[[6.87473461365077,45.9772995683242,2517.2578125],[6.8736121249999,45.9779670155837,2542.50024414063],[6.87299210781629,45.9781103097697,2579.26586914063]]])
